  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Tallinna Tehnika OÜ"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Tallinna Tehnika OÜ" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest signing-method-invalid-is-held-and-unoverridable
  (testing "declared signing method not in Estonia's closed eID set -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
...
(deftest vat-record-unverified-is-held-and-unoverridable
  (testing "unverified EMTA VAT-liability record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:vat-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
