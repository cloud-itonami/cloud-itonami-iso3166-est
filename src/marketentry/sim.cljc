...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (EST, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up signing-method-invalid) ==")
    ...
    (println "== filing/submit eng-4 (signing-method-invalid -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up vat-record-unverified) ==")
