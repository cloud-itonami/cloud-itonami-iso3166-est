(ns marketentry.governor
  ...
  actuation is always a human call.
    ...
    3. Signing-method invalid      -- for `:filing/submit`,
                                       ...
                                       belongs to Estonia's closed set
                                       of nationally-recognised
                                       e-identification methods, and
                                       ...
                                       build time) -- a CLOSED-SET
                                       VALIDITY test on the FILING'S
                                       OWN EXECUTION INSTRUMENT (was it
                                       digitally signed with a method
                                       Estonia's e-government
                                       infrastructure actually
                                       recognises at all), a check
                                       AXIS genuinely different from
                                       ...
                                       first in this family to gate on
    ...
    5. VAT record unverified        -- for `:filing/submit`, when the
                                       ...
                                       the Estonian Tax and Customs
                                       Board (EMTA/MTA) VAT-liability
                                       registration obligation, Value
...
(defn- signing-method-invalid-violations
  ...
  engagement's own declared digital-signing method belongs to
  Estonia's closed set of nationally-recognised e-identification
  methods -- the flagship check this vertical adds. HARD-hold when the
  engagement's declared `:signing-method` is not a member of
  `marketentry.registry/valid-signing-methods`."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (registry/signing-method-invalid-claim? e)
        [{:rule :signing-method-invalid
          :detail (str subject " の申告署名方式(" (:signing-method e)
                      ")はエストニアの認定電子本人確認手段(ID-card/e-Resident's digital ID/Smart-ID/Mobile-ID)の"
                      "閉集合に属さない -- 独立再計算により提出提案は進められない")}]))))
...
(defn- vat-record-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-vat-record? true`, INDEPENDENTLY check
  `:vat-record-verified?` -- CONDITIONAL on the engagement's own
  ground truth. Grounded in the Estonian Tax and Customs Board (EMTA/
  MTA), Value Added Tax Act subsection 3 of section 19^1."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-vat-record? e))
                 (not (true? (:vat-record-verified? e))))
        [{:rule :vat-record-unverified
          :detail (str subject " はEMTA(Maksu- ja Tolliamet)VAT登録記録の確認を要するが未確認 -- 提出提案は進められない")}]))))
