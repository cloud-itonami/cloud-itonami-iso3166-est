...
2. Register the client's intake: business type, target public function,
   prior filing history in Estonia if any.
3. Run the advisor in read-only mode against the Riigihangete Register
   (riigihanked.riik.ee).
4. Compare the checklist against the client's current documentation
   (e-Business Register status, e-ID/digital-signature capability).
...
- named referral relationship with Estonian-licensed counsel or a
...
a working referral relationship with Estonian-licensed counsel or a
registered agent for whatever licensed representation the law of
Estonia requires for actual public-procurement filings.
